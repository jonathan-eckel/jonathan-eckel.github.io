...
{% img /images/beersomme_input.png %}
...
{% img /images/beersomme_output.png %}

The output page provides a list of nearby bars that have beers similar to "Dogfish Head 60 Minute IPA" available on tap.  The top two recommendations are to check out **Professor Thom's** and **Taproom No. 307**.  Each bar is recommended because each has two beers that are of similar style, IPA or Pale Ale --- beers on the hoppy/bitter side of things.  The process for determining beer similarity will be discussed [below](#beer-similarity). For additional information about the bar, the user can click on the Foursquare link in the top right corner of each listing.

## Technology Behind the Beer

So how does it work? Let's venture a bit deeper...

The app consists of three parts:

1. Local bar/beerlist data collection and lookup
2. Recommendations based on beer descriptions
3. Front end

Below is a schematic of the data flow with methods and technologies shown:
{% img /images/beersomme_stack.png %}

In essence, **beersomme** is combining two beer data sources to provide meaningful recommendations on where to find your next beer.
...
In order to limit the number of API calls the app makes to Untappd, I institute a caching mechanism to store bar and beer information locally using a [MySQL](http://www.mysql.com/) database.  Therefore, the app first checks whether there is sufficient information in the local database before making additional API calls.
...
- If not enough bars and beers cached:
...
I used the webapp from [Kimono Labs](http://www.kimonolabs.com/) to automate the scraping task of extracting the description from each beer URL.
...
Each beer can now represented as a list of word-occurence pairs, such as {hops: 2, malt: 1, ...}.  This is known as a [bag-of-words](http://en.wikipedia.org/wiki/Bag-of-words_model) model.  Each description is transformed into a list of word counts, where word ordering no longer matters.  To utilize ordering information, bigrams and higher order n-grams, could prove useful for future iterations of **beersomme**.

Next, I remove all words that will not help in representing beers.  These include all words that only occur once throughout the _corpus_, the entire body of text. I also remove _stop words_, the common glue words in sentences.

Now, I can represent each beer as a sparse vector of word counts.  The vector is $$n$$-dimensional, where $$n$$ is the number of unique words left in the corpus.  The values in each dimension correspond to the frequency of the word in each beer description.  In order to deal with additional common words that may pop up (beer, malt, etc...), I use a [tf-idf](http://en.wikipedia.org/wiki/Tf%E2%80%93idf) (term frequency -- inverse document frequency) weighting. This down-weights words that occur frequently throughout the corpus.  Conversely, it up-weights words that occur rarely in individual beer descriptions.  Performing this analysis on my corpus of beers results in the creation of a _term-document_ matrix, or in my case, a _keyword-beer_ matrix.  This is a sparse matrix with dimension: $$\rm nwords ~\times ~ \rm nbeers$$.

<!--
The app is recommending we check out Professor Thom's, where they have two beers similar on tap:
...
After text processing, I have beer vectors in word space.  Next, I need to choose a distance metric to determine which beers are closest to each other.  Since the vectors are normalized (from tf-idf) I use a [cosine similarity](http://en.wikipedia.org/wiki/Cosine_similarity) distance metric.  So distance is measured by the angle of separation between the two vectors as follows:
  
$$
\cos(\theta) = \frac{A \cdot B}{\lvert A \rvert ~ \lvert B \rvert}
$$

{% img right /images/cosine_similarity.svg 400 300 %}
So if I have two beers, $$A$$ and $$B$$, they can be represented in some word space as shown here.  They are separated by some angle 
$$\theta$$.  Since these word vectors are positive (there can be no negative counts of word appearance), the angle $$\theta$$ varies between $$[0^\circ, 90^\circ]$$ and correspondingly $$\cos(\theta)$$ varies between $$[1, 0]$$.

Acting on each pair of beers, we can create a beer similarity matrix, where $$A_{ij}$$ corresponds to the similarity between beer i & j.
...
The webpages are built from [Bootstrap](http://getbootstrap.com/) templates.  The site is deployed on [Amazon Web Services](http://aws.amazon.com/) in an EC2 instance.


## Validation

Besides using the eye test (or taste test in this case) we need to validate that the recommender is working properly.  First, let's ask, can I use a bag-of-words method to distingish beers?  Can I use a simple bag-of-words method to identify beers?  Will this provide sufficient information to separate disparate beer styles? 
...
- **Top left:** refreshing, crisp
- **Bottom left:** hoppy, bitter
- **Bottom right:** dark, rich

Given the localized clusters and the separation between each cluster, we can see that the beer description data are robust.  

## Closing Thoughts

This project was built in three weeks as an [Insight Data Science] [Insight] Fellow.
This was my first go at using text as the primary form of data.  Building this project allowed me to learn a great deal about Natural Language Processing.  If you have any questions about this project feel free to contact me at [jonathan.eckel@gmail.com](mailto:jonathan.eckel@gmail.com) or tweet me at [@jonathan_eckel](http://twitter.com/Jonathan_Eckel).
